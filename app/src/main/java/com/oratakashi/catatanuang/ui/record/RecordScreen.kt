...
/**
 * Record Finance screen composable for adding a new financial transaction.
 *
 * @author oratakashi
 * @since 22 Mar 2026
 * @param onNavigateBack Callback to navigate back.
 * @param viewModel The [RecordViewModel] injected via Koin.
 */
...
fun RecordScreen(
    ...
    val judulError = judul.isBlank() && uiState is RecordUiState.Error
    val nominalError = (nominalText.digitsOnly().toLongOrNull() ?: 0L) <= 0L && uiState is RecordUiState.Error

    LaunchedEffect(uiState) {
        when (val state = uiState) {
            is RecordUiState.Success -> onNavigateBack()
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(stringResource(R.string.title_catat_keuangan)) },
                navigationIcon = {
        ...
        Column(
            ...
            Button(
                ...
                modifier = Modifier.fillMaxWidth(),
                enabled = uiState !is RecordUiState.Loading
            ) {
                if (uiState is RecordUiState.Loading) {
